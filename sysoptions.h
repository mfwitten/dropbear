...
#if (DROPBEAR_SVR_PUBKEY_AUTH) && (DROPBEAR_SVR_PUBKEY_OPTIONS)
	#define DROPBEAR_SVR_PUBKEY_OPTIONS_BUILT 1
#else
...
#if (DROPBEAR_SVR_PUBKEY_AUTH) || (DROPBEAR_CLIENT)
	#define DROPBEAR_SIGNKEY_VERIFY 1
#else
	#define DROPBEAR_SIGNKEY_VERIFY 0
#endif
...
#if (DROPBEAR_SHA2_256_HMAC) || (DROPBEAR_ECC_256) || (DROPBEAR_CURVE25519) || (DROPBEAR_DH_GROUP14_SHA256)
	#define DROPBEAR_SHA256 1
#else
	#define DROPBEAR_SHA256 0
#endif

#define DROPBEAR_SHA384 DROPBEAR_ECC_384

/* LTC SHA384 depends on SHA512 */
#if (DROPBEAR_SHA2_512_HMAC) || (DROPBEAR_ECC_521) || (DROPBEAR_SHA384) || (DROPBEAR_DH_GROUP16)
	#define DROPBEAR_SHA512 1
#else
	#define DROPBEAR_SHA512 0
#endif
...
#if (DROPBEAR_AES256) || (DROPBEAR_AES128)
	#define DROPBEAR_AES 1
#else
	#define DROPBEAR_AES 0
#endif

#if (DROPBEAR_TWOFISH256) || (DROPBEAR_TWOFISH128)
	#define DROPBEAR_TWOFISH 1
#else
...
#else
	#define DROPBEAR_CLI_ANYTCPFWD 0
#endif

#if (DROPBEAR_CLI_LOCALTCPFWD) || (DROPBEAR_SVR_REMOTETCPFWD)
	#define DROPBEAR_TCP_ACCEPT 1
#else
	#define DROPBEAR_TCP_ACCEPT 0
#endif

#if (DROPBEAR_CLI_REMOTETCPFWD) || (DROPBEAR_CLI_LOCALTCPFWD) || (DROPBEAR_SVR_REMOTETCPFWD) \
      || (DROPBEAR_SVR_LOCALTCPFWD) || (DROPBEAR_SVR_AGENTFWD) || (DROPBEAR_X11FWD)
	#define DROPBEAR_LISTENERS 1
#else
	#define DROPBEAR_LISTENERS 0
#endif

#if (DROPBEAR_CLI_NETCAT) && (DROPBEAR_CLI_PROXYCMD)
	#define DROPBEAR_CLI_MULTIHOP 1
#else
	#define DROPBEAR_CLI_MULTIHOP 0
#endif

#if (DROPBEAR_CLI_AGENTFWD) || (DROPBEAR_USE_PRNGD)
	#define ENABLE_CONNECT_UNIX 1
#else
	#define ENABLE_CONNECT_UNIX 0
#endif

/* if we're using authorized_keys or known_hosts */ 
#if (DROPBEAR_CLIENT) || (DROPBEAR_SVR_PUBKEY_AUTH)
	#define DROPBEAR_KEY_LINES 1
#else
	#define DROPBEAR_KEY_LINES 0
#endif
